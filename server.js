...
matrix = generate(25, 15, 10, 9, 0, 1);
...
function jsonn(){
    let jsonData = `{"Spawned":[{"name":"Grass","number":"${grassQAN}"},{"name":"Grass Eater","number":"${grassetQAN}"}, {"name":"Predator","number":"${predatorQAN}"}, {"name":"Virus","number":"${virusQAN}"}]}\n`;
...
io.on('connection', function (socket) {
    ...
    socket.on("button", (stream) => {
        ...
        if (stream == "gr") {
            ...
            goblin = []
            virus = []
            matrix = generate(15, 15, 10, 9, 0, 1)
        }
        io.sockets.emit("send matrix", matrix);
    })
    socket.on("grassQAN", (stream) => {
        grassQAN = stream;
        jsonn()
    })
    socket.on("grassetQAN", (stream) => {
        grassetQAN = stream;
        // jsonn()
    })
    socket.on("predatorQAN", (stream) => {
        predatorQAN = stream;
        // jsonn()

    })
    socket.on("virusQAN", (stream) => {
        virusQAN = stream;
        // jsonn()
    })
})
